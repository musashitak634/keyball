...
static void motion_to_mouse_move(keyball_motion_t *m, report_mouse_t *r, bool is_left) {
    adjust_mouse_speed(m); // Add this line to adjust the mouse speed
#if KEYBALL_MODEL == 61 || KEYBALL_MODEL == 39 || KEYBALL_MODEL == 147 || KEYBALL_MODEL == 44
    r->x = clip2int8(m->y);
    r->y = clip2int8(m->x);
    if (is_left) {
...
// Add these includes at the beginning
#include "keyball.h"

// Replace the existing motion_to_mouse_move function with the updated one
static void motion_to_mouse_move(keyball_motion_t *m, report_mouse_t *r, bool is_left) {
    adjust_mouse_speed(m); // Add this line to adjust the mouse speed
#if KEYBALL_MODEL == 61 || KEYBALL_MODEL == 39 || KEYBALL_MODEL == 147 || KEYBALL_MODEL == 44
    r->x = clip2int8(m->y);
